...
class PriorityQBinHeap(object):
    ...
    def pop(self):
        ...
        while (i * 2) < len(self._tree_list) - 1:
            ...
            if self._tree_list[swap_child][1] < self._tree_list[i][1] if self.min_heap else self._tree_list[swap_child][1] > self._tree_list[i][1]:
    ...
    def push(self, value, priority=0):
        ...
        if len(self._tree_list) > 1:
            ...
            while i > 0:
                parent = (i - 1) // 2
                if self._tree_list[i][1] < self._tree_list[parent][1] if self.min_heap else self._tree_list[i][1] > self._tree_list[parent][1]:
                    self._swap(parent, i)
    ...
    def _eval_child(self, i):
        ...
        if i * 2 + 2 > len(self._tree_list) - 1:
            ...
            if self._tree_list[i * 2 + 1][1] < self._tree_list[i * 2 + 2][1] if self.min_heap else self._tree_list[i * 2 + 1][1] > self._tree_list[i * 2 + 2][1]:
                return i * 2 + 1
            else:
                return i * 2 + 2
